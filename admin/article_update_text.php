...
if (isset($_POST['partition'])) {$heading_r=$_POST['partition'];       if($heading_r ==''){unset($heading_r);}}
...
if (isset($title) && isset($meta_key) && isset($meta_desc) && isset($date) && isset($description) && isset($text) && isset($author)&& isset($heading_r) && isset($icon)&& isset($resolution))
{
$itog=mysql_query("UPDATE note SET title='$title', meta_key='$meta_key', meta_desc='$meta_desc', date='$date', description='description', text='$text', author='$author', heading_r='$heading_r', icon='$icon', resolution='$resolution' WHERE id='$id'");
if ($itog=='true'){
echo "<p>ваш статья успешно обнавлена в базе</p>";} 
else {echo "<p>ваш статья не обнавлен в базе</p>";}
}	
else
{
echo"<p>вы ввели не полностью информацию,поэтому статья в базу не может быть обнавлена</p>";
}		
